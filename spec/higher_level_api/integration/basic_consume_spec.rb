...
describe "A consumer" do
  ...
  context "instantiated with an explicitly provided executor (thread pool)" do
    it "uses said executor" do
      ch = connection.create_channel
      ...
      consumer1        = q.subscribe_with(consumer_object, :blocking => false, :executor => HotBunnies::ThreadPools.fixed_of_size(16))
      consumer2        = q.subscribe(:blocking => false, :executor => HotBunnies::ThreadPools.dynamically_growing) do |metadata, payload|
        mailbox2 << payload
        all_received.count_down
      end
      consumer3        = q.subscribe(:blocking => false, :executor => HotBunnies::ThreadPools.single_threaded) do |metadata, payload|
        mailbox3 << payload
        all_received.count_down
      end
